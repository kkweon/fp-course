...
runTestJsonParser :: IO ()
runTestJsonParser = defaultMain test_JsonParser
...
jsonNumberTest =
  testGroup
    ...
    , testCase "negative whole" $
      parse jsonNumber "-234" @?= Result "" ((-234) % 1)
    , testCase "positive decimal" $
      parse jsonNumber "123.45" @?= Result "" (2469 % 20)
    , testCase "negative whole (2)" $
      parse jsonNumber "-123" @?= Result "" ((-123) % 1)
    , testCase "negative decimal" $
      parse jsonNumber "-123.45" @?= Result "" ((-2469) % 20)
    , testCase "negative sign on its own is error" $
      isErrorResult (parse jsonNumber "-") @?= True
    , testCase "alphabetic characters is error" $
      isErrorResult (parse jsonNumber "abc") @?= True
    ]
...
jsonTrueTest =
  testGroup
    ...
    , testCase "TRUE (caps) is an error" $
      isErrorResult (parse jsonTrue "TRUE") @?= True
    ]
...
jsonFalseTest =
  testGroup
    ...
    , testCase "FALSE (caps) is an error" $
      isErrorResult (parse jsonFalse "FALSE") @?= True
    ]
...
jsonNullTest =
  testGroup
    ...
    , testCase "NULL (caps) is an error" $
      isErrorResult (parse jsonNull "NULL") @?= True
    ]
  ...
  testGroup
    ...
    , testCase "[true, \"abc\", [false]]" $
      let result =
            Result
              ""
              (JsonTrue :. JsonString "abc" :. JsonArray (JsonFalse :. Nil) :.
               Nil)
