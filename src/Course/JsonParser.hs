...
toSpecialCharacter c =
  let table =
        ('b', BackSpace) :. ('f', FormFeed) :. ('n', NewLine) :.
        ('r', CarriageReturn) :.
        ('t', Tab) :.
        ('v', VerticalTab) :.
        ('\'', SingleQuote) :.
...
jsonString = between (is '"') (charTok '"') (list _parser)
  where
    _parser :: Parser Char
    _parser = do
      c <- character
      if c == '\\'
        then handleSpecialChar
        else handleNoSepcialChar c
    handleSpecialChar :: Parser Char
    handleSpecialChar = do
      nextChar <- character
      if nextChar == 'u'
        then hex
        else handleNonHex nextChar
    handleNonHex :: Char -> Parser Char
    handleNonHex c =
      case toSpecialCharacter c of
        Full specialChar -> return $ fromSpecialCharacter specialChar
        Empty -> unexpectedCharParser c
    handleNoSepcialChar :: Char -> Parser Char
    handleNoSepcialChar '"' = unexpectedCharParser '"'
    handleNoSepcialChar c = return c
...
jsonNumber =
  P (\input ->
       case readFloats input of
         ...
         Full (value, rest) -> Result rest value)
...
jsonObject = betweenSepbyComma '{' '}' _parser
  where
    _parser :: Parser (Chars, JsonValue)
    _parser = (,) <$> jsonString <* charTok ':' <*> jsonValue
...
jsonValue =
  ...
  (JsonNull <$ jsonNull |||
   ...
   JsonObject <$> jsonObject ||| JsonRational <$> jsonNumber)
