...
P f ||| P g =
  P (\input ->
       let firstTry = f input
        in if isErrorResult firstTry
             then g input
             else firstTry)
...
instance Monad Parser where
  ...
  f =<< P g = P $ \input -> onResult (g input) (\next a -> parse (f a) next)
...
satisfy p = P go
  where
    ...
    go input@(x :. _) =
      if p x
        then parse character input
        else parse (unexpectedCharParser x) input
...
sequenceParser = foldRight folder (valueParser Nil)
  where
    folder :: Parser a -> Parser (List a) -> Parser (List a)
    folder p ps = do
      a <- p
      listA <- ps
      return $ a :. listA
...
surnameParser = do
  u <- upper
  lowerChars <- thisMany 5 lower
  remainingChars <- list lower
  return $ u :. lowerChars ++ remainingChars
...
phoneParser =  do
  firstDigit <- digit -- necessary to avoid "-123-456"
  phoneNumber <- phoneBodyParser
  is '#'
  return $ firstDigit :. phoneNumber
...
-- Result >< Person 123 "Fred" "Clarkson" True "123-456.789"
--
-- Age FirstName Surname smoke phone text
...
personParser = do
  ...
  spaces1
  Person age firstName surname isSmoker <$> phoneParser
