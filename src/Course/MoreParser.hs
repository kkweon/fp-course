...
-- True
--
-- traverse :: (a -> Parser Char) -> List Char -> Parser (List Char)
...
noneof chars = satisfy (`notElem` chars)
...
between openP closeP middle = do
  openP
  text <- middle
  closeP
  return text
...
hex :: Parser Char
hex = do
  fourChars <- replicateA 4 (satisfy isHexDigit)
  case readHex fourChars of
    Full value -> return (chr value)
    Empty -> fail "Fail"
...
sepby1 valueParser' sepParser = do
  value <- valueParser'
  next <- list (sepParser >> valueParser')
  return $ value :. next
...
sepby valueParser' sepParser = sepby1 valueParser' sepParser ||| pure Nil
...
betweenSepbyComma openChar closeChar givenParser =
  betweenCharTok openChar closeChar (sepby givenParser commaTok)
