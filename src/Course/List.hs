...
filter f = foldRight filterFn Nil
  where
    filterFn value container
      | f value = value :. container
      | otherwise = container
...
first ++ second = foldRight (:.) second first
...
seqOptional = foldRight go (Full Nil)
  where
    go :: Optional a -> Optional (List a) -> Optional (List a)
    go Empty _ = Empty
    go _ Empty = Empty
    go (Full a) (Full xs) = Full (a :. xs)
...
find f = foldRight finder Empty
  where
    finder value optional
      | f value = Full value
      | otherwise = optional
...
reverse = foldLeft go Nil
  where
    go container value = value :. container
